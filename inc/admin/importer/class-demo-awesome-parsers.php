...
/**
 * Demo Awesome importer class for managing parsing of WXR files
 */
if ( ! class_exists( 'Demo_Awesome_Parser' ) ) {
	class Demo_Awesome_Parser {
		function parse( $file ) {
			// Attempt to use proper XML parsers first
			if ( extension_loaded( 'simplexml' ) ) {
				$parser = new Demo_Awesome_Parser_SimpleXML;
				...
				$parser = new Demo_Awesome_Parser_XML;
				$result = $parser->parse( $file );
			...
			if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
				...
				if ( 'SimpleXML_parse_error' == $result->get_error_code() ) {
					foreach ( $result->get_error_data() as $error ) {
						echo wp_kses_post($error->line . ':' . $error->column . ' ' . esc_html( $error->message ) . "\n");
					}
				...
				echo '</pre>';
				echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'demo-awesome' ) . '</strong><br />';
				echo esc_html__( 'Details are shown above. The importer will now try again with a different parser...', 'demo-awesome' ) . '</p>';
			}

			// use regular expressions if nothing else available or this is bad XML
			$parser = new Demo_Awesome_Parser_Regex;
 ...
 */
if ( ! class_exists( 'Demo_Awesome_Parser_SimpleXML' ) ) {
	class Demo_Awesome_Parser_SimpleXML {
		function parse( $file ) {
			...
			if ( ! $success || isset( $dom->doctype ) ) {
				return new WP_Error( 'SimpleXML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'demo-awesome' ), libxml_get_errors() );
			}
			...
			if ( ! $wxr_version ) {
				return new WP_Error( 'WXR_parse_error', esc_html__( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'demo-awesome' ) );
			}
 ...
 */
if ( ! class_exists( 'Demo_Awesome_Parser_XML' ) ) {
	class Demo_Awesome_Parser_XML {
		var $wp_tags = array(
			'wp:post_id',
			'wp:post_date',
			'wp:post_date_gmt',
 ...
 */
if ( ! class_exists( 'Demo_Awesome_Parser_Regex' ) ) {
	class Demo_Awesome_Parser_Regex {
		var $authors = array();
		var $posts = array();
