...
if ( ! class_exists( 'Demo_Awesome_Admin' ) ) {
	class Demo_Awesome_Admin {
		...
		function call_import_function_from_ajax() {
			...
			wp_send_json_success( array(
				'success' => true,
				'message' => sprintf( __( '<div>%s<span class="mr-1">%s</span>%s%s%s</div>' ), '<h3>', Demo_Awesome_Admin::get_svg( 'check' ), __( 'Import finished', 'demo-awesome' ), '</h3>', __( 'The demo has been imported successfully', 'demo-awesome' ) )
			) );
